window.addEventListener("DOMContentLoaded",function(){setUpNavClickHandlers();setUpCarousels();setUpFaqQuestions();setUpDesktopMoreFeaturesButton();setUpMobileMoreFeaturesButtons();});function setUpNavClickHandlers(){document.getElementById('nav-open').onclick=function(){document.getElementById("nav-menu").classList.toggle("hidden")
...
function setUpDesktopMoreFeaturesButton(){if(window.location.pathname!="/"){return}
const desktopMoreFeaturesButton=document.getElementById('desktopMoreFeatures');desktopMoreFeaturesButton.addEventListener('click',function(){const rowsToShow=document.querySelectorAll('#pricing table tr.hidden');rowsToShow.forEach(function(rowToShow){rowToShow.classList.remove('hidden');});desktopMoreFeaturesButton.parentElement.classList.add('hidden');});}
